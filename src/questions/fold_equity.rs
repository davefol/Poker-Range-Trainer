use iced::{
    Row, text_input, button, Text, TextInput, Button, Length, Align, Container, Column
};
    ...
    fn view(&mut self) -> Row<Message> {
       Row::new()
           ...
           .push(
               ...
               .align_items(Align::Center)
               .spacing(100)
                   .push(Text::new(format!("Hero risk {} BB to win {} BB.\nWhat is the required fold equity?", self.risk, self.reward)))
                   .push(TextInput::new(
                           &mut self.ans_input, 
                            "RFE", 
                            &self.user_ans, 
                            |s| Message::StudyScreen(StudyMessage::QuestionAnswer(QuestionAnswer::FoldEquity(FoldEquityMessage::TextChanged(s)))))
                        .width(Length::Units(80))
                        .size(32)
                       )
                   .push(
                       match self.state {
                          QuestionState::Waiting => Button::new(&mut self.submit_button, Text::new("Submit"))
                             .on_press(Message::StudyScreen(StudyMessage::QuestionAnswer(QuestionAnswer::FoldEquity(FoldEquityMessage::Submit)))),
                          QuestionState::Correct => Button::new(&mut self.next_button, Text::new(format!("Correct: {} BB", self.ans)))
                              .on_press(Message::RequestNewQuestion),
                          QuestionState::Wrong => Button::new(&mut self.next_button, Text::new(format!("Wrong: {} BB", self.ans)))
                              .on_press(Message::RequestNewQuestion),
                       }
                         )
               )
